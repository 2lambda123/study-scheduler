...
if(isset($_SESSION['access_token']) && $_SESSION['access_token'] != ""){ //check if $SESSION is true -> user already logged in and want to continue with fb
  $result = $db -> select("SELECT FBAUTH FROM user WHERE ID='9e08a9cf-33c3-11e7-8e1a-00ff89cf7daa'");
    if(quote($fbId) == $result && $result != ""){ //FBAUTH already in database, dont do anything
  }
    ...
    $db -> query("UPDATE user SET FBAUTH=".$db->quote($fbId)." WHERE ID='9e08a9cf-33c3-11e7-8e1a-00ff89cf7daa'");
  }
}

...
if($finalStudyTimes != null){
  ...
  foreach ($studyDecoded as $study){
	...
	if($i < count($study)){
      echo "<h4>Study with " . $study[0]["name"] . " - Course: " . $study[0]["course"] . "</h4>";
	  for ($i; $i < count($study); $i++){
      if(strcmp(substr($study[$i]["DTSTART"], 0, 8), substr($study[$i]["DTEND"], 0, 8)) === 0){
        echo substr(convertPrintTime($study[$i]["DTSTART"]), 0, 10) . "&nbsp &nbsp &nbsp" . substr(convertPrintTime($study[$i]["DTSTART"]), -5, 5)." - ";
        echo substr(convertPrintTime($study[$i]["DTEND"]), -5, 5). "<br>";
      }
      else{

...
function analyze ($events) {
	...
	for ($i = 0; $i < $count; $i++) {
		if ($e[$i]->AVAILABLE) {
			if (days($e[$i]->DTSTART, $collection) && days($e[$i]->DTEND, $collection)) { //Om ett event bara pÃ¥gÃ¥r under dagar man ej vill plugga
				unset($e[$i]);
				$e = array_values($e);
			} else {
				$date = $e[$i]->DTSTART;
				if(days($e[$i]->DTSTART, $collection)) { //Om ett event bÃ¶rjar pÃ¥ en dag man ej vill plugga
					$e[$i]->DTSTART = date('Ymd', strtotime($e[$i]->DTSTART .' +1 day')) . "T0000Z";//$e[$i]->DTSTART = next day 00.00;
				}
				$date = $e[$i]->DTSTART;
				for ( ; $date < $e[$i]->DTEND; ) {
					if (days($date, $collection)) {
						if (substr($e[$i]->DTEND, 0, 8) == substr($date, 0, 8) && substr($e[$i]->DTSTART, 0, 8) == substr($date, 0, 8)) {  //Om ett event bara pÃ¥gÃ¥r under en dag man ej vill plugga
						 unset($e[$i]);
						 $e = array_values($e);
						...
						else { //Annars, om ett event bÃ¶rjar en dag man ej vill plugga
							$e[$i]->DTSTART = date('Ymd', strtotime($e[$i]->DTSTART .' +1 day')) . "T0000Z";//$e[$i]->DTSTART next day 00.00
						}
	...
	for ($i = 0; $i < $count; $i++) {
		if ($e[$i]->AVAILABLE) {
			...
			if ($sleepfrom < $sleepto) { //Om man sover frÃ¥n tidigt pÃ¥ morgonen till senare pÃ¥ dagen
				...
				if ($e[$i]->DTSTART < $sleepfrom && $e[$i]->DTEND > $sleepfrom && $e[$i]->DTEND <= $sleepto) { //Om ett event bÃ¶rjar innan man sover och slutar under tiden man sover
					$e[$i]->DTEND = $sleepfrom;
				} else if ($e[$i]->DTSTART >= $sleepfrom && $e[$i]->DTEND <= $sleepto) { //Om ett event bara gÃ¥r medans man sover
					unset($e[$i]);
					$e = array_values($e);
				} else if ($e[$i]->DTSTART >= $sleepfrom && $e[$i]->DTSTART < $sleepto && $e[$i]->DTEND > $sleepto) { //Om ett event bÃ¶rjar innan man vaknar och fortsÃ¤tter efter man vaknat
					$e[$i]->DTSTART = $sleepto;
				} else if ($e[$i]->DTSTART < $sleepfrom && $e[$i]->DTEND > $sleepto) { //Om ett event bÃ¶rjar innan man somnar och slutar efter men vaknat
					...
					$avEvent->UID = $e[$i]->UID; //fixa unik id nå² vi skaffat databas
